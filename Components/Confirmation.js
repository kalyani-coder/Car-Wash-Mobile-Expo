...
const Confirmation = ({ route, navigation }) => {
  ...
  const handleSubmit = async () => {
    if (validateFields()) {
      ...
      const { pickupAddress, date, time, servicesName, status, price, serviceImage } = route.params;
  ...
  return (
    <KeyboardAvoidingView
      style={[styles.header, commonStyles]}
    ...
    >
      ...
      >
        ...
        <TextInput
          ...
          value={clientcarmodelno.toUpperCase()}
          style={styles.input}
        />
...
const styles = StyleSheet.create({
  ...
  container: {
    paddingTop: 15,
    padding: 15,
    flexGrow: 1, // Allow the content to grow when the keyboard is open
  },
  text: {
    fontSize: 15,
    fontWeight: "bold",
